...
return new class extends Migration
{
    ...
    {
        // Create a new table with auto-increment ID
        Schema::create('cars_old', function (Blueprint $table) {
            $table->id();
            $table->string('user_id', 6);
            $table->foreign('user_id')->references('userId')->on('users')->onDelete('cascade');
            $table->string('name_of_owner');
        ...
        // Copy data back (losing UUIDs, generating new auto-increment IDs)
        $cars = DB::table('cars')->get();
        foreach ($cars as $car) {
            DB::table('cars_old')->insert([
                'user_id' => $car->user_id,
                'name_of_owner' => $car->name_of_owner,
                'phone_number' => $car->phone_number,
                'address' => $car->address,
